...
int main(int argc, char *argv[])
{
    ...
    // argv : The array which store all of the arguments
    ...
    while (file >> header)
    {
        // ref_list[1] starts to store the reference sequence
        ref_list[ref_num] = header;
        // first line is .benchmark testcase1
        // make the title correct 
        if (!cout_start)
        {
            file >> header;
            ref_list[ref_num] += " " + header;
